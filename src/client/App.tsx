...
export default function App() {
  ...
  const selfId = participants.filter((participant) => participant.name === name).at(-1)?.id;
  ...
  useEffect(() => {
    ...
    roomClient.connect(roomId, name).catch((cause: unknown) => setError(cause instanceof Error ? cause.message : 'A conexão foi interrompida.'));
    return () => { unsubscribe(); roomClient.close(); };
  }, [roomId, name]);

  const handleMessage = (message: ServerMessage) => {
    if (message.type === 'snapshot') { setParticipants(message.participants); return; }
    ...
    if (message.type === 'participant-left') {
      ...
      setRemoteStreams((current) => { const next = new Map(current); next.delete(message.participantId); return next; });
      return;
    }
